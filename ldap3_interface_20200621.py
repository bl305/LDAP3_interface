...
class LDAP_interface():
    ...
    def ldap_query_csv(self, pquery, pdelimiter=','):
    ...
    def ldap_query_value(self, pquery):
        myconn = self.ldap_connect()
        query_res = self.ldap_query(pconn=myconn, pquery=pquery)
        retval = str()
        try:
...
if __name__ == "__main__":
    ...
    if args['param_queryvalue']:
        output = aconnection.ldap_query_value(pquery=aquery)

    if args['param_querycsv']:
        output = aconnection.ldap_query_csv(pquery=aquery)
